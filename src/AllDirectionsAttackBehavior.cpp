...
//-----functions section------
//-----------------------------------------------------------------------------
void AllDirectionsAttackBehavior::Attack(sf::Vector2f position, sf::Vector2f direction, std::vector<std::unique_ptr<Bullets>>& bullets, BulletOwner owner)
{
    ...
    for (int directionBullet = 0; directionBullet < NUM_OF_DIRECTIONS; directionBullet++)
    {
        ...
        auto bullet = std::make_unique<Bullets>(position, bulletDirection, 300.0f, owner);
